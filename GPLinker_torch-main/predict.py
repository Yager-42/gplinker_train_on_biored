...
with open("./datasets/pubmed-LiverNeopl-set.txt",'r',encoding="utf-8") as f,open("./CMeIE_test.json", 'w', encoding="utf-8") as wr:
    text_list = []
    pattern = 'AB  - (.*?)\.\n'
    textRe = re.compile(pattern,flags=re.DOTALL)
    all = f.read()
    result = textRe.findall(all)
    for data in result:
        text_list.append(' '.join(data.replace('\n','').split()))
    for text in text_list:
        ...
        for spo in list(spoes):
            spo_list.append({"subject":spo[0], "subject_type":spo[1].split("_")[0],
                             "predicate":spo[1].split("_")[1:-1],
                             "object":{"@value":spo[2]}, "object_type": {"@value": spo[1].split("_")[-1]}
                             })
        wr.write(json.dumps({"spo_list":spo_list}, ensure_ascii=False))
        wr.write("\n")
