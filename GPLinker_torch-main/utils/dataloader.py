...
def find_name(entities,id):
    for entity in entities:
        if id in entity['concept_id']:
            type=''
            if 'Gene' in entity['semantic_type_id']:
                type = 'Gene'
            elif 'Variant' in entity['semantic_type_id']:
                type = 'Variant'
            elif 'Species' in entity['semantic_type_id']:
                type = 'Species'
            elif 'Disease' in entity['semantic_type_id']:
                type = 'Disease'
            elif 'Chemical' in entity['semantic_type_id']:
                type = 'Chemical'
            elif 'CellLine' in entity['semantic_type_id']:
                type = 'CellLine'
            return [entity['text'][0], type]

def process_raw_data():
    raw_data = load_dataset("bigbio/biored",split="train")
    D = []
    for data in raw_data:
        relations = []
        entities = data['entities']
        for relation in data['relations']:
            entity_id1 =relation['concept_1']
            entity_id2 = relation['concept_2']
            entity1 = find_name(entities,entity_id1)
            entity2 = find_name(entities,entity_id2)
            if entity1 == None or entity2 == None:
                continue
            rel = {'subject' : entity1[0], 'predicate' : relation['type'], 'object' : entity2[0], 'subject_type' : entity1[1], 'object_type' : entity2[1]}
            relations.append(rel)
        D.append({
            "text" : data['passages'][1]['text'][0],
            "spo_list" : [(spo["subject"], spo["predicate"], spo["object"], spo["subject_type"], spo["object_type"])
                            for spo in relations]
        })
    raw_data = load_dataset("bigbio/biored",split="test")
    for data in raw_data:
        relations = []
        entities = data['entities']
        for relation in data['relations']:
...
if __name__=='__main__':
    load_name(r"C:\Users\RR\Desktop\GPLinker_torch-main\datasets\train.JSON")
